    ...
    ForEach($Mill in $Mills) {
        ...
        ForEach($UserProfile in $UsersProfiles) {
            ...
            If (Test-Path "$($UserProfile)\Guichet\GS_StayOP.ini") {
                ...
                $GS_StayOP = Get-Content "$($UserProfile)\Guichet\GS_StayOP.ini" | Select-String -Pattern "Program="
                ForEach ($Program in $GS_StayOP){
                    If ($Program -notmatch "REM "){
                        If (($Program.Line.Split("=")[1]) -match "\\"){
                            write-host $Program.Line.Split("=")[1]
                            Out-File -FilePath "C:\Scripts\Benoit\GS_StayOP_Program.txt" -InputObject $Program.Line.Split("=")[1] -Append
                        }
                    }
                }
