...
$O365CustomerName      = "Cascades"    #This should be the name of your tenant (example, ogd as in ogd.onmicrosoft.com) 
...
$desiredMappings =  @(
    @{"displayName"="Onedrive Cascades";"targetLocationType"="driveletter";"targetLocationPath"="V:";"sourceLocationPath"="autodetect";"mapOnlyForSpecificGroup"=""}
    #@{"displayName"="Onedrive Cascades";"targetLocationType"="driveletter";"targetLocationPath"="V:";"sourceLocationPath"="autodetect";"mapOnlyForSpecificGroup"=""}
#   @{"displayName"="Onedrive Cascades";"targetLocationType"="networklocation";"targetLocationPath"="$env:APPDATA\Microsoft\Windows\Network Shortcuts";"sourceLocationPath"="autodetect";"mapOnlyForSpecificGroup"=""}
)
...
$favoriteSitesDLName   = "Shared  Documents"       #Normally autodetected, default document library name in Teams/Groups/Sites to map in conjunction with $autoMapFavoriteSites, note the double spaces! Use Shared  Documents for english language tenants
...
$persistentMapping     = $False                    #If set to $False, the mapping will go away when the user logs off
...
$showProgressBar       = $False                    #will show a progress bar to the user
...
$onedriveIconPath = "\\ad.cascades.com\apps\CTX-Apps\OneDriveMapper\onedrive.ico" #if this file exists, and you've set addShellLink to True, it will be used as icon for the shortcut
...
# CASCADES - 05-07-2019 - Save Date and UserPrincipalName on FS-01-CTI server to analsysis
function SSPR_Report {
    param (
        [Parameter(Mandatory=$true)][String]$UserACC,
        [String]$Error
    )
    $TimeStamp = Get-Date -f 'yyyy-MM-dd HH:mm'
    $Share = "\\fs-01-cti\sspr_report$"
    $Global:csv = @()

    $MemberDetails = New-Object PSObject -Property @{
        'Date' = $TimeStamp
        'UserPrincipalName' = $UserACC
        'SamAccountName' = $($env:USERNAME)
        'Error' = $Error
    }
    $Global:csv += $MemberDetails 

    $Global:csv | Select Date,UserPrincipalName,SamAccountName,Error | Export-Csv -NoTypeInformation -Encoding UTF8 -Path "$Share\SSPR_Report.csv" -Append
}

# CASCADES - 06-28-2019 - Create registry key to define OneDrive as icon image
function iconDrive{
    Param( 
        [String]$lD_DriveLetter,
        [String]$onedriveIconPath
    )
    #try to set the drive icon
    log -text "A drive icon has been specified, attempting to set the icon for $($lD_DriveLetter)"
    $lD_DriveLetter_Reg = $lD_DriveLetter -replace ":",""
    $path = "HKCU:\Software\Classes\Applications\Explorer.exe\Drives\$($lD_DriveLetter_Reg)\DefaultIcon"
    if(!(Test-Path $path)){
        try{ 
            $Null = New-Item -Path $path -Force -ErrorAction SilentlyContinue
            $Null = Set-Item -Path $path -Value $onedriveIconPath -ErrorAction SilentlyContinue
            log -text "Drive icon has been set to $($onedriveIconPath)"
         }catch{ 
            log -text "Failed to set the drive icon registry keys: $($Error[0]) " -fout
        }
    }else{
        log -text "Map drive icon is already configured" -fout
    }
}
...
function CustomInputBox(){ 
    ...
    $objBalloon.BalloonTipText = "OneDriveMapper" 
    $objBalloon.Visible = $True  
    $objBalloon.ShowBalloonTip(10000) 
...
function abort_OM{ 
    ...
    if($displayErrors){
        if($errorsForUser){ 
            ...
            #$OUTPUT2= [System.Windows.Forms.MessageBox]::Show("Message Cascades - You can always use https://portal.office.com to access your data", "Need a workaround?" , 0)
            #$OUTPUT2= [System.Windows.Forms.MessageBox]::Show("You need to configure SSPR (Self-Service Password Reset) questions to map OneDrive in your Citrix Session. Please contact IT Cascades to more information.", "IT Cascades" , 0) 
            
            #CASCADES - CALL TO SSPR_RERPOT FUNCTION
            SSPR_Report -UserACC $userUPN -Error $errorsForUser
        }
    }
    Exit
} 
...
if($authMethod -ne "native"){
    ...
    if($res -eq $False){
        if($allowFallbackMode -and !$fallbackMode){
            ...
            $errorsForUser += "CASCADES - SSPR NOT CONFIGURED"
            abort_OM
        }
    }else{
...
Get-PSDrive -PSProvider filesystem | Where-Object {$_.DisplayRoot} | % {
    if($_.DisplayRoot.StartsWith("\\$($O365CustomerName).sharepoint.com") -or $_.DisplayRoot.StartsWith("\\$($O365CustomerName)-my.sharepoint.com")){
        try{$del = NET USE "$($_.Name):" /DELETE /Y 2>&1;Write-host "Success delete mapped drive."}catch{Write-host "Error delete mapped drive."}     
    }
}

#clean up empty mappings
Get-PSDrive -PSProvider filesystem | Where-Object {($_.Used -eq 0 -and $_.Free -eq $Null)} | % {
    try{$_ | Remove-PSDRive -Force;Write-host "Success delete empty drive."}catch{Write-host "Error delete empty drive."}     
}
...
#map the drives
foreach($mapping in $desiredMappings){
    #CASCADES - CALL FONCITON ICONDRIVE
    iconDrive -lD_DriveLetter $($mapping.targetLocationPath) -onedriveIconPath $($onedriveIconPath)
