...
ForEach($Company in $Companies){
    ...
    ForEach($Mill in $Mills) {
        ...
        ForEach($UserProfile in $UsersProfiles) {
            ...
            If (Test-Path "$($UserProfile)\Guichet") {
         
                $ChildItems = Get-ChildItem -Path "$($UserProfile)\Guichet" -Filter "*.lnk" -Recurse 
                ForEach ($Item in $ChildItems) {
                    $Item.fullname | Out-File "C:\Scripts\Benoit\Shortcut.txt" -Append
